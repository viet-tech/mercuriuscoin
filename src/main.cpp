...
unsigned int nTargetSpacing = 7 * 60; // 7 mins
unsigned int nRetarget = 70;
unsigned int nStakeMinAge = 11 * 60; // 11 mins
unsigned int nStakeMaxAge = -1;  // 
unsigned int nModifierInterval = 10 * 60; 

int nCoinbaseMaturity = 1; //1+10
...
const string strMessageMagic = "Mercurius Signed Message:\n";
...
static const int64_t nTargetTimespan = 70 * 60;  // Retarget Difficulty every 70 minutes
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Mercurius", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nCoinbaseMaturity = 1; // test maturity is 1 + 10 blocks
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Mercurius Coin 2017";
        CTransaction txNew;
        txNew.nTime = 1501471064 ;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
	...
	if (false && (block.GetHash() != hashGenesisBlock)) {
		...
		printf("New Genesis Hash for Mercurius Coin \n");
	...
	assert(block.hashMerkleRoot == uint256("0xa28ddaded46de47e64f6b43e276fbb997fbebf2d6dfd1c3a5d40683368a5e8f4"));
